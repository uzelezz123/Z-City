...
local teams = {
	[0] = {
		objective = "Find all the hiders before the time runs out!",
		name = "Seeker",
		color1 = Color(245, 66, 90),
		color2 = Color(245, 66, 90)
	},
	[1] = {
		objective = "Run around in circles and hide! Your life depends on it!",
		name = "Hider",
...
function MODE:RenderScreenspaceEffects()
	...
	-- FIXME: Shitty work around, we should try something better :/
	...
	elseif team_ == 1 then
		...
		fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
	else
		if zb.ROUND_START + 7.5 < CurTime() then return end
	end
...
function MODE:HUDPaint()
	...
	if timeBeforeSWAT > 0 and zb.ROUND_START + 10.5 < CurTime() then
		...
		local w2, h2 = surface.GetTextSize("11:11:11 time left before rescue arrives!")
	...
	if zb.ROUND_START + 40 > CurTime() then
		...
		draw.SimpleText( "Seekers will spawn in: "..string.FormattedTime(zb.ROUND_START + 40 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.95, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText( "Seekers will spawn in: "..string.FormattedTime(zb.ROUND_START + 40 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.95) - 2, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		local fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
		surface.SetDrawColor(0, 0, 0, 255 * fade)
		surface.DrawRect(-1, -1, ScrW() + 1, ScrH() + 1)
	end
...
local CreateEndMenu
net.Receive("hs_roundend", function() CreateEndMenu(net.ReadBool()) end)
