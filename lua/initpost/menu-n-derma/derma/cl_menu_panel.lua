...
local red_select = Color(255,123,0)

local Selects = {
    {Title = "Выйти", Func = function(luaMenu) RunConsoleCommand("disconnect") end},
    {Title = "Старое меню", Func = function(luaMenu) gui.ActivateGameUI() luaMenu:Close() end},
    {Title = "Настройки", Func = function(luaMenu) luaMenu:Close() RunConsoleCommand("hg_settings") end},
    {Title = "Стим коллекция", Func = function(luaMenu) luaMenu:Close() gui.OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=3658085621")  end},
    {Title = "Дискорд", Func = function(luaMenu) luaMenu:Close() gui.OpenURL("https://discord.gg/EBceTtbFC8")  end},
    ...
    {Title = "Гардероб", Func = function(luaMenu) luaMenu:Close() RunConsoleCommand("hg_appearance_menu") end},
...
function PANEL:InitializeMarkup()
    ...
    local text = "<font=ZC_MM_Title><colour=255,91,15>HOMI<colour=2255,145,0>BURG</font>\n<font=ZCity_Small>" .. gm .. "</font>"
    return markup.Parse(text)
end
...
function PANEL:AddSelect( pParent, strTitle, tbl )
    ...
    function btn:DoClick()
        surface.PlaySound("homigrad/vgui/csgo_ui_store_select.wav")
        btn.Func(luaMenu)
    end
    ...
    function btn:Think()
        ...
        if isHovered and not hasHovered then
            hasHovered = true
            surface.PlaySound("homigrad/vgui/buttonrollover.wav")
